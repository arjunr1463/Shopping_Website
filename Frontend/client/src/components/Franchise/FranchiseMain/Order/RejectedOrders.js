...
function RejectedOrders() {
  ...
  const fetchData = async () => {
    ...
    axios
      ...
      .then((res) => {
        const filter = res.data.filter((data) => data.status === "Rejected");
        setTableData(filter);
        console.log(filter);
      });
  };
  ...
  return (
    <div className="flex flex-col gap-[25px]">
      ...
      <div className="mx-[10px] sm:mx-[20px] flex flex-col gap-[10px]">
        <h className="font-mont font-semibold text-[22px]">Rejected orders</h>
                  ...
                  ? paginatedData.map((data, index) => (
                      <tr key={index}>
                        ...
                        <td className="border-[1px] text-[#189279]">
                          <Link
                            to={`/franchise-rejectedconfirm/${data.checkout_id}`}
                          >
                            View
                          </Link>
                        </td>
